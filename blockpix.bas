...
DO:BP.RESIZE:W=BP.WIDTH():H=BP.HEIGHT():FOR Y,0,Y<H,1:FOR X,0,X<W,1:BP.IMMEDIATE_SET X,Y,CINT(RAND(ABS(SIN(X)),ABS(COS(Y*PI()/2)))*3)/3*(1-(X/W/1.5))*255:NEXT:NEXT:LOOP
#
