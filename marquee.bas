...
do
    ...
    if colors<>0
        ...
        color oc:?
        r=r+x:g=g+y:b=b+z
        if r>255:x=x*-1:endif:if r<0:x=x*-1:endif
        if g>255:y=y*-1:endif:if g<0:y=y*-1:endif
        if b>255:z=z*-1:endif:if b<0:z=z*-1:endif
    else
        ?"\r";snip$(str$,0,w)
    endif
    if d$="R":str$=snip$(str$,len(str$)-1,len(str$))+snip$(str$,0,len(str$)-1):else:str$=snip$(str$,1,len(str$))+snip$(str$,0,1):endif
    waitms limit(delayms-timerms(),0,delayms)
    if _os$()="Windows":if inplace<>0:locate ,cy:else:locate ,cy+1:cy=cury():endif:else:if inplace<>0:rlocate ,-1:endif:endif
